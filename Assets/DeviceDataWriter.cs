...
public class DeviceDataWriter : MonoBehaviour
{
    private string m_dataFolderPath;
    private string m_dataFileName = "VRDeviceData";
    ...
    public void SaveExperimentData(string data)
    {
        ...
        task.Result.Wait();
#else
        string fileName = m_dataFileName + System.DateTime.Now.ToString("yyyyMMMMdd HH") + m_dataExtension;
        Stream stream = new FileStream(Path.Combine(m_dataFolderPath, fileName), FileMode.Append, FileAccess.Write);
